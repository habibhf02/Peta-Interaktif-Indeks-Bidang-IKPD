...
if (indikator === "ALL") {
  // cek apakah ADA minimal 1 data tersedia
  const semuaIndikator = data ? Object.values(data) : [];

  nilai = semuaIndikator.some(ind => {
    const v = ind?.[tahun];
    return v !== undefined && !isNaN(v);
  });

} else {
  nilai = data?.[indikator]?.[tahun];
}

    // 🔥 jika tidak ada data
    if (
  indikator === "ALL"
    ? !nilai   // nilai sekarang boolean (true/false)
    : (nilai === undefined || isNaN(nilai))
  ) {
